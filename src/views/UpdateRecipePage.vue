            ...
            <td>
              <div id="newRecipeInstructions">
                <input
                  ...
                  type="text"
                  :placeholder="i"
                  v-model="newRecipeInstructions[i - 1]"
                />
                <br />
                <button
                ...
                <tr v-for="i in noOfIngredients">
                  <td>
                    <input
                      ...
                      v-model="this.newRecipeIngredients[i - 1]"
                    />
                  </td>
                  <td>
                    <input
                      ...
                      v-model="this.newRecipeIngredientsMeasure[i - 1]"
                    />
                  </td>
                </tr>
                <br />
  ...
  methods: {
    ...
    getRecipeIngredients() {
      ...
      for (let i = 0; i < this.noOfIngredients; i++) {
        let ingredient =
          tempRecipeIngredients[i].getElementsByTagName("input")[0].value;
        let measure =
          tempRecipeIngredientsMeasure[i].getElementsByTagName("input")[0]
            .value;
    ...
    viewRecipeDetails() {
      ...
      console.log(
        "newRecipeIngredientsMeasure: " + this.newRecipeIngredientsMeasure
      );
    ...
    updateDatabase() {
      ...
      if (this.newRecipeImage != null) {
        ...
        storage
          .uploadBytes(storageRef, this.newRecipeImage)
          .then((snapshot) => {
            ...
            storage.getDownloadURL(storageRef).then((downloadURL) => {
              ...
              for (let i = 1; i < this.newRecipeIngredients.length + 1; i++) {
                ...
                fullFoodTemplate[measure] =
                  this.newRecipeIngredientsMeasure[i - 1];
              }
                          ...
                          .then((snapshot) => {
                            ...
                            for (let i in data) {
                              ...
                              if (data[i].id == this.id) {
                                let updateRef = ref(
                                  db,
                                  `users/${userId}/recipes/${i}`
                                );
                                remove(updateRef).then(() => {
                                  console.log("Location removed");
                                  axios
                                    .post(
                                      `https://esd-uchef-restore-default-rtdb.asia-southeast1.firebasedatabase.app/users/${this.$store.state.userId}/recipes.json`,
                                      simpleFoodTemplate
                                    )
                                    .then((response) => {
